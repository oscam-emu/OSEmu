...
KeyData *EmuKeys = NULL;
unsigned int keyCount = 0;
unsigned int keyMax = 0;
...
void SetKey(char identifier, unsigned int provider, char *keyName, unsigned char *key, unsigned int keyLength) 
{
  ...
  memcpy(EmuKeys[keyCount].keyName, keyName, 8);
  EmuKeys[keyCount].keyName[7] = 0;
  EmuKeys[keyCount].key = key;
  EmuKeys[keyCount].keyLength = keyLength;  
  keyCount++;
}
...
// Cryptoworks EMU
static char crw_0D00C000[]={0x22,0xEC,0xB8,0xB2,0x43,0x85,0xC3,0xB2,0x94,0x1E,0xF7,0xEC,0xC2,0xB5,0x4A,0x09}; // DigiTurk 7°E/42°E
static char crw_0D00C001[]={0xC0,0xC6,0x86,0x55,0xB6,0x33,0x04,0x08,0x5B,0x5F,0x72,0x2D,0xD4,0x71,0xFE,0x27}; // DigiTurk 7°E/42°E
static char crw_0D00C006[]={0x01,0x56,0x12,0xE8,0xEE,0x33}; // DigiTurk 7°E/42°E
static char crw_0D00C400[]={0x22,0xEC,0xB8,0xB2,0x43,0x85,0xC3,0xB2,0x94,0x1E,0xF7,0xEC,0xC2,0xB5,0x4A,0x09}; // DigiTurk 7°E/42°E
static char crw_0D00C401[]={0xC0,0xC6,0x86,0x55,0xB6,0x33,0x04,0x08,0x5B,0x5F,0x72,0x2D,0xD4,0x71,0xFE,0x27}; // DigiTurk 7°E/42°E
static char crw_0D00C406[]={0x01,0x56,0x12,0xE8,0xEE,0x33}; // DigiTurk 7°E/42°E
...
static char crw_0D02A801[]={0xC2,0x29,0xA4,0x03,0x61,0x00,0xE8,0x1A,0x42,0xBA,0xC3,0xC7,0x0E,0xBB,0xC8,0x52}; // UPC 0.8°W
static char crw_0D030400[]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x19,0xE8,0xF5,0x24,0xC6,0xC0,0x53,0xE4}; // Sky Link 23.5°E
static char crw_0D030406[]={0x27,0xB4,0x88,0x34,0x8D,0x54}; // Sky Link 23.5°E
static char crw_0D030801[]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x42,0xF0,0x7B,0xE9,0x20,0xC7,0xE0,0xF3}; // Sky Link 23.5°E
static char crw_0D030806[]={0x27,0xB4,0x88,0x34,0x8D,0x54}; // Sky Link 23.5°E
static char crw_0D050400[]={0xA9,0xD7,0x32,0xF5,0xE7,0x38,0xE4,0x8A,0x91,0xC8,0x63,0x8B,0x12,0x74,0x66,0x61}; // ORF 19.2°E
static char crw_0D050406[]={0x09,0x87,0xCF,0x2A,0x61,0x1D}; // ORF 19.2°E
static char crw_0D051000[]={0x00,0x00,0x6A,0xB3,0x25,0xE6,0xAB,0xF4,0xDA,0x47,0x28,0x0F,0x35,0x29,0x4B,0x5C}; // Austriasat 19.2°E
static char crw_0D051006[]={0x09,0x87,0xCF,0x2A,0x61,0x1D}; // ORF  19.2°E
...
char GetCwKey(unsigned char *buf,unsigned int ident, unsigned char keyIndex, unsigned int keyLength) {
  ...
  switch ((ident << 8) + keyIndex) {
    ...
    case 0x0D00C006 : memcpy(buf,crw_0D00C006,6);  return 1; // DigiTurk 7°E/42°E
    case 0x0D00C400 : memcpy(buf,crw_0D00C400,16);  return 1; // DigiTurk 7°E/42°E
    case 0x0D00C401 : memcpy(buf,crw_0D00C401,16);  return 1; // DigiTurk 7°E/42°E
    case 0x0D00C406 : memcpy(buf,crw_0D00C406,6);   return 1; // DigiTurk 7°E/42°E
    ...
    case 0x0D050400 : memcpy(buf,crw_0D050400,16);  return 1; // ORF 19.2°E
    case 0x0D050406 : memcpy(buf,crw_0D050406,6);  return 1; // ORF 19.2°E
    case 0x0D051000 : memcpy(buf,crw_0D051000,16);  return 1; // Austriasat 19.2°E
    case 0x0D051006 : memcpy(buf,crw_0D051006,6);   return 1; // Austriasat 19.2°E
  }
  return 0;
}
...
{
  int i, j;
  unsigned char key[16], desKey[16], t[8], dat1[8], dat2[8];
  static const char k0D00C000[]={0x22,0xEC,0xB8,0xB2,0x43,0x85,0xC3,0xB2,0x94,0x1E,0xF7,0xEC,0xC2,0xB5,0x4A,0x09};
  memset(t, 0, 8);
...
static char nagra_C102_M1[]={0x31, 0x22, 0xD1, 0xC2, 0x29, 0x07, 0xB3, 0x98, 0xDC, 0x25, 0x3F, 0xEA, 0x29, 0xD0, 0x33, 0x85, 0x97,
  ...
  0x31, 0x10, 0x0C, 0x2A, 0xA0}; // TV Globo 13.0°E
static char nagra_C102_00[]={0x5B, 0xCE, 0x65, 0x62, 0x5F, 0xA4, 0xED, 0x50, 0x95, 0x94, 0xCE, 0x7C, 0x4B, 0x90, 0x7E, 0xCB}; // TV Globo 13.0°E
static char nagra_C102_01[]={0xD7, 0xCB, 0xE9, 0x3D, 0x30, 0xE2, 0xC9, 0x13, 0x91, 0x07, 0x38, 0x74, 0x57, 0xDB, 0x90, 0x23}; // TV Globo 13.0°E
...
char GetNagraKey(unsigned char *buf, unsigned int ident, char keyName, unsigned int keyIndex)
{
  ...
  switch(ident) {
    case 0x2111:  // Digi TV Cablu (Romanian Cable TV)
      ...
      if(keyName == '0' && keyIndex == 0x00) { memcpy(buf,nagra_2111_00,16); return 1; }
      if(keyName == '0' && keyIndex == 0x01) { memcpy(buf,nagra_2111_01,16); return 1; }                        
      break;        
    case 0x7301:  // UPC Cablecom (Swiss Cable TV) 
      if(keyName == 'M' && keyIndex == 0x01) { memcpy(buf,nagra_7301_M1,64); return 1; }
      if(keyName == '0' && keyIndex == 0x00) { memcpy(buf,nagra_7301_00,16); return 1; }
      if(keyName == '0' && keyIndex == 0x01) { memcpy(buf,nagra_7301_01,16); return 1; }                        
      break;  
    case 0xC102:  //  TV Globo 13.0°E
      if(keyName == 'M' && keyIndex == 0x01) { memcpy(buf,nagra_C102_M1,64); return 1; }
      if(keyName == '0' && keyIndex == 0x00) { memcpy(buf,nagra_C102_00,16); return 1; }
      if(keyName == '0' && keyIndex == 0x01) { memcpy(buf,nagra_C102_01,16); return 1; }                        
      break;  
    case 0x1101:  // Kabel Deutschland
      if(keyName == 'M' && keyIndex == 0x01) { memcpy(buf,nagra_1101_M1,64); return 1; }
      if(keyName == '0' && keyIndex == 0x00) { memcpy(buf,nagra_1101_00,16); return 1; }                    
      break;              
  }
  return 0;
}
...
char GetIrdetoKey(unsigned char *buf, unsigned int ident, char keyName, unsigned int keyIndex)
{
  ...
  switch(ident) {
    case 0x60400:  // Bulsat 39°E
      ...
      if(keyName == '0' && keyIndex == 0x01) { memcpy(buf,irdeto_60400_01,16); return 1; }
      if(keyName == '0' && keyIndex == 0x02) { memcpy(buf,irdeto_60400_02,16); return 1; }                        
      if(keyName == '0' && keyIndex == 0x03) { memcpy(buf,irdeto_60400_03,16); return 1; }
      if(keyName == '0' && keyIndex == 0x04) { memcpy(buf,irdeto_60400_04,16); return 1; }  
      if(keyName == '0' && keyIndex == 0x05) { memcpy(buf,irdeto_60400_05,16); return 1; }
      if(keyName == '0' && keyIndex == 0x06) { memcpy(buf,irdeto_60400_06,16); return 1; }        
      break;              
  }
  return 0;
}
